...
```
@Alice to create user stories for chat feature
@Bob will update architecture doc
```

**Pattern 2: Name + action verb**
```
Alice to create user stories
Bob will update architecture doc
Carol should review the mockups
```

**Pattern 3: Action: Name - Task**
```
Action: Alice - create user stories
Action Item: Bob - update architecture
```

**Pattern 4: TODO with assignee**
```
TODO: Create user stories (Alice)
TODO: Update docs - Bob
```

**Pattern 5: Bullet with name**
```
- Alice: create user stories
- Bob - update architecture
```
...
Action Items:
- @Alice to create user stories for chat feature
- Bob will update the architecture doc
- Carol: review and approve design mockups
```
...
```
✅ Found: Alice Johnson (alice.johnson@example.com)
→ Use accountId from result
```

**Scenario B: No Match (0 results)**
```
⚠️ Couldn't find user "Alice" in Jira.

Options:
1. Create task unassigned (assign manually later)
2. Skip this task
3. Try different name format (e.g., "Alice Johnson")
...
```
⚠️ Found multiple users named "Alice":
1. Alice Johnson (alice.johnson@example.com)
2. Alice Smith (sarah.smith@example.com)
...
- Try full name first ("Alice Johnson")
- If no match, try first name only ("Alice")
...
**Source:** https://example.atlassian.net/wiki/spaces/TEAM/pages/12345
...
```
Notes mention: @sarah, Alice, Alice J.

These likely refer to the same person. I'll look up "Alice" once and use 
...
1. "@Alice to create user stories" (line 15)
2. "Action: Alice - create user stories" (line 42)
...
- @Alice to create user stories for chat feature
- @Bob will update the architecture doc
- @Carol should review design mockups
```

**Process:**
1. Parse → 3 action items found
2. Project → "PROJ"
3. Lookup → Alice (123), Bob (456), Carol (789)
...
Follow-ups:
- Alice will draft the PRD
- Bob: implement API changes
- TODO: Review security audit (Carol)
...
- @Alice to refactor code
```

**Process:**
1. Parse → 2 action items
2. Lookup "John" → Found 3 Johns!
3. Ask → "Which John? (Dave Smith, Dave Doe, Dave Wilson)"
4. User → "Dave Smith"
